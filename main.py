...
@bot.slash_command(name='scripts',
                   description='Categorized Roblox script pack')
async def scripts(ctx):
    await scripts_cmd(ctx, bot)


@bot.slash_command(name='search',
                   description='Searches for scripts by game name')
async def search(ctx, name):
    await search_cmd(ctx, bot, name)


@bot.slash_command(name='ban',
                   description='Bans specified user for breaking the rules')
